...
class GODataset1(Dataset):
    ...
    def protein_to_graph(self, pFilePath, amino, pos_ca_ori):
        ...
        #     with open(pFilePath) as f:
        #         pdb_str = f.read()
        #     parser = pdb_tools.PDBParser()
        #     pdb_tools.repair(pdb_str, parser)  # 使用repair()方法修复PDB文件
        #     # 现在可以解析pdb_str了
        #     pdb_structure = parser.get_structure('protein', pdb_str)  
        ...
        # h5File.close()
            ...
            bb_embs = torch.full((len(amino),6), torch.nan)
            # print(pFilePath)
            # print(torch.sum(bb_embs))
            return side_chain_embs, bb_embs
            

class ECDataset1(Dataset):
