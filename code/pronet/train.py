...
def main():
    ...
    parser.add_argument('--dataset_path', type=str, default='/usr/data/protein/ProtFunct/', help='path to load and process the data')
    ...
    if args.dataset == 'func': #ER reaction
        try:
            train_set = ECdataset(root=args.dataset_path, split='Train')
            val_set = ECdataset(root=args.dataset_path, split='Val')
            test_set = ECdataset(root=args.dataset_path, split='Test')
    ...
    elif args.dataset == 'fold': #fold:family,super
        try:
            train_set = FOLDdataset(root=args.dataset_path, split='training')
            val_set = FOLDdataset(root=args.dataset_path, split='validation')
            test_fold = FOLDdataset(root=args.dataset_path, split='test_fold')
            test_super = FOLDdataset(root=args.dataset_path, split='test_superfamily')
            test_family = FOLDdataset(root=args.dataset_path, split='test_family')
        except FileNotFoundError: 
            print('\n Please download data firstly, following https://github.com/divelab/DIG/tree/dig-stable/dig/threedgraph/dataset#ecdataset-and-folddataset and https://github.com/phermosilla/IEConv_proteins#download-the-preprocessed-datasets \n')
            raise(FileNotFoundError)
        train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
